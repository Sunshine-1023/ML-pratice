...
## 本机环境说明（PyTorch 2.x / Apple Silicon / 无 torchtext）

项目已改为 **不依赖 torchtext**，使用 `torch.utils.data.DataLoader` 完成数据加载与批处理，兼容 PyTorch 2.x。

建议环境：

1. Python 3.10+（推荐 conda 或 venv）  
2. 安装依赖：

```bash
pip install -r 情感分类实战/requirement.txt
```
